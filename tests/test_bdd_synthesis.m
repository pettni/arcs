...
function test_pre(testCase)
	...
	s1 = TransSyst(6, 2, 'bdd', 'log');
  s2 = TransSyst(6, 2, 'bdd', 'split');
	for s = [s1, s2]
    ...
    verifyEqual(testCase, uint32(s.pre(X, U, true, true))', uint32([3, 4, 5, 6]))
    verifyEqual(testCase, uint32(s.pre(X, U, true, false))', uint32([3, 6]))
    verifyEqual(testCase, uint32(s.pre(X, U, false, true))', uint32([5, 6]))
    verifyEqual(testCase, uint32(s.pre(X, U, false, false))', uint32([6]))
  end
end

function test_until(testCase)
	s1 = TransSyst(5, 1, 'bdd', 'log');
  s2 = TransSyst(5, 1, 'bdd', 'split');
  for s = [s1, s2]
    ...
    verifyEqual(testCase, s.win_until(1:5, [3], false), [2, 3, 4]')
    
    s.add_progress_group([1], [1, 2]);
    
    verifyEqual(testCase, s.win_until(1:5, [3], false), [1, 2, 3, 4]')
    
    verifyEqual(testCase, s.win_primal(1:5, [3, 5], {1:5}, 'exists', 'forall'), (1:5)');
...
function test_primal2(testCase)
	s1 = TransSyst(5, 2, 'bdd', 'log');
  s2 = TransSyst(5, 2, 'bdd', 'split');
  for s = [s1, s2]
    s.add_transition(1, 2, 1);
...
function test_primal3(testCase)
	s1 = TransSyst(7, 2, 'bdd' ,'log');
  s2 = TransSyst(7, 2, 'bdd', 'split');
