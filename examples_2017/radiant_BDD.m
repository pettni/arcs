...
while true
  ...
  if measure_mem
    ...
    mem = part.ts.bdd_sys.measure_memory();
    data(iter+1, :) = [iter, time, prime_time, split_time, length(part), sum(volume(part.cell_list(Win)))/volume(part.domain), nodes, sys_nodes, mem];
  else
  ...
  if measure_mem
    disp(['Total nodes: ', num2str(nodes), ', system nodes: ', num2str(sys_nodes), ', Memory: ', num2str(mem/1024), 'kB']);
  end
  % Solve <>[] 'SET'
