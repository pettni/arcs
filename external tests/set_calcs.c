...
//      uint state_num = 0;
//
//      uint* in_states = malloc(sizeof(uint)*max_trans_num);
//      uint** outputs = malloc(sizeof(uint*)*2);
//      outputs[0] = &state_num;
//      outputs[1] = in_states;
//
//      DdGen* gen;
...
//      uint state_num = 0;
//
//      uint* out_states = malloc(sizeof(uint)*max_state_num);
//      uint** outputs = malloc(sizeof(uint*)*2);
//      outputs[0] = &state_num;
...
//      uint action_num = 0;
//
//      uint* a_states = malloc(sizeof(uint)*max_trans_num);
//      uint** outputs = malloc(sizeof(uint*)*2);
            ...
            char quant, int mode)
{
    ...
    if (Cudd_EquivDC(sys->manager, quick_test, Cudd_ReadLogicZero(sys->manager), Cudd_ReadLogicZero(sys->manager)))
    {
        ...
        DdNode** out = malloc(sizeof(DdNode*)*mode);
        if (mode >= WIN_SET)
        {
            out[0] = Y;
            if (mode >= WIN_CANDIDATE_SET)
...
DdNode** PGpre(BDDSys* sys, DdNode* Z, DdNode* B, char quant, int mode)
{
    ...
    for (int i = 0; i < array_len(sys->pg_U); i++)
    {
        ...
        {
            printf("This happens! Progress group skipped\n");
            continue;
        }
                         ...
                         DdNode** C, int C_num, char quant, int mode)
{
    DdNode* W;
    DdNode* old_W;
    DdNode** Z_iter = (DdNode**)malloc(sizeof(DdNode*)*C_num);
    DdNode* tmp;
    DdNode* pre_set;
    DdNode* one = Cudd_ReadOne(sys->manager);
    Cudd_Ref(one);
